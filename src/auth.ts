...
export async function verifyPassword(db: Supabase, username: string, password: string, clientIp: string): Promise<string> {
  ...
  if (data.access_token) {
    // The launcher uses our revocable Yggdrasil token, not a persistent Supabase session.
    await db.request('/auth/v1/logout?scope=local', {
      method: 'POST', headers: { Authorization: `Bearer ${data.access_token}` },
    }, false).catch(() => {});
  }
  if (!identity || data.user?.id !== identity.user_id) throw invalidCredentials();
